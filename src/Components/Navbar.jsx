...
const Navbar = () => {
  ...
  return (
    <>
      ...
      >
        {/* Brand Section */}
        <div className="text-3xl font-black flex items-center gap-1 cursor-pointer group relative"
             style={{
               background: mode === 'light'? 'linear-gradient(to right, #66ffff, #f9fbe7)'
                                        : 'linear-gradient(to right, #2c3e50, #4ca1af)',
               padding: '0.05rem',
               borderRadius: '10px',
           }}>
          <a href='#Home' className='flex  gap-3'>
           <span className="text-xl tracking-wide" style={{ color: mode === 'light' ? '#000' : '#fff' }}>Ankur</span>
           <FaLaptopCode className="text-3xl" style={{ color: mode === 'light' ? '#1e90ff' : '#ff6347' }} />
          </a>
        </div>
          ...
          >
            ...
            <span
              className={`absolute bottom-0 left-0 h-[2px] bg-gradient-to-r ${
                mode === "light" ? "from-blue-500 to-green-500" : "from-pink-500 to-red-500"
      ...
      <div
        className={`fixed top-0 left-0 w-full h-full  ${mode === 'light' ? 'bg-slate-100':'bg-black'} bg-opacity-90 transition-all duration-500 ease-in-out transform ${
          isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
        } z-40 md:hidden`}
      >
      <ul
      className="flex flex-col items-center justify-center text-xl space-y-6"
      style={{ height: '100vh', padding: '20px', background: mode === 'light' ? '#f9f9f9' : '#333', transition: 'background-color 0.3s ease' }}
    >
      <li
        className={`cursor-pointer hover:text-red-500 ${mode === 'light' ? 'text-black' : 'text-white'} 
